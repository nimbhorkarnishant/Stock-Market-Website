...
<body>
	...
	font-weight: bold; width: 100%; padding:2px; background-color: #34495E  ; font-size: 1.2rem; ">
				    Company Valuation Information History
				  </div>
				  <div class="card-body">
            ...
            if ($user_detail->num_rows > 0) {
              echo '
					         <thead class="table-dark">
					            <tr>
                        <th scope="col">ID</th>
                        <th scope="col">Valuation ID</th>
                        <th scope="col">Company ID</th>
        					      <th scope="col">Market Cap </th>
        					      <th scope="col">p_per_e</th>
                        <th scope="col">book_value</th>
                        <th scope="col">dividend</th>
                        <th scope="col">market_lot</th>
                        <th scope="col">industry_p_per_e</th>
        					      <th scope="col">eps</th>
                        <th scope="col">p_per_c</th>
                        <th scope="col">price_per_book</th>
                        <th scope="col">dividend_yield</th>
                    ...
                    while($row = $user_detail->fetch_assoc()) {
                      //  echo "Id: " . $row["Id"]. " - Name: " . $row["first_name"]. " " . $row["last_name"]. $row["email"].$row["branch"].$row["block"].$row["gender"].$row["event"];
                        ...
                        echo "<tbody>";
                          echo "<tr>";
                          echo  "<td>".$row["Id"]."</td>";
													echo  "<td>".$row["valuation_id"]."</td>";
                          echo  "<td>".$row["company_id"]."</td>";
                          echo  "<td>".$row["market_cap"]."</td>";
                          echo  "<td>".$row["p_per_e"]."</td>";
                          echo  "<td>".$row["book_value"]."</td>";
                          echo  "<td>".$row["dividend"]."</td>";
                          echo  "<td>".$row["market_lot"]."</td>";
                          echo  "<td>".$row["industry_p_per_e"]."</td>";
                          echo  "<td>".$row["eps"]."</td>";
                          echo  "<td>".$row["p_per_c"]."</td>";
                          echo  "<td>".$row["price_per_book"]."</td>";
                          echo  "<td>".$row["dividend_yield"]."</td>";
                          echo  "<td>".$row["face_value"]."</td>";
