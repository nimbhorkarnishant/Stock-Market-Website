...
<body>
  ...
  ?>
	...
	font-weight: bold; width: 65vw; padding:2px; background-color: #34495E  ; font-size: 1.2rem; ">
				  ...
				  <div class="card-body">
				  	<form class="form-inline" action="?act=filter_by_company_finanace_id" method="post">
          ...
          <?php
            if ($company_financial_detail->num_rows > 0) {
              echo '
					         <thead class="table-dark">
					            <tr>
        					      <th scope="col">Company ID</th>
        					      <th scope="col">Finance Field</th>
        					      <th scope="col">Quarter 1</th>
                        <th scope="col">Quarter 2</th>
        					      <th scope="col"></th>
					             </tr>
					          </thead>';
                    while($row = $company_financial_detail->fetch_assoc()) {
                      //  echo "Id: " . $row["Id"]. " - Name: " . $row["first_name"]. " " . $row["last_name"]. $row["email"].$row["branch"].$row["block"].$row["gender"].$row["event"];
                        ...
                        //  echo  "<td>".$row["company_code"]."</td>";
                          echo  '<td><form  action="?act=view_company_finanacial_detail" method="post">'.
                                  '<button class="btn-sm btn-warning" type="submit" name="financial_id" value='.$id.">"."View Details"."</button>"
                                  .'</form></td>';
                          echo "</tr>";
                        echo "</tbody>";
