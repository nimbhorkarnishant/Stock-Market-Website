            ...
            <div class="form-group mx-sm-3 mb-2">
              <form class="" action="?act=filter_by_user_type" method="post">
                <label for="inputEmail3" class="sr-only"></label>
					    <input type="text" name="filter_by" class="form-control" id="inputEmail3" placeholder="Search User" style="width:19%">
					  </div>
					  ...
					  <a href="?act=add_new_user"><img src="wt_project/user/views/media/website_images/add_user.svg" width="70px;" height="50px;" style="margin-left: 15vw;"> Add User</a>
					</form>
					<br>
					<br>
                    ...
                    while($row = $user_detail->fetch_assoc()) {
                      //  echo "Id: " . $row["Id"]. " - Name: " . $row["first_name"]. " " . $row["last_name"]. $row["email"].$row["branch"].$row["block"].$row["gender"].$row["event"];
                        ...
                        echo "<tbody>";
                          ...
                          echo  '<td><form  action="?act=view_user_detail" method="post">'.
                                  '<button class="btn-sm btn-warning" type="submit" name="user_id" value='.$id.">"."View Details"."</button>"
                                  .'</form></td>';
                          echo "</tr>";
                        echo "</tbody>";
