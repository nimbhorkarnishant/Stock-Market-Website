...
<body>
  ...
  <a href="?act=add_new_company_detail" style="float:right;margin-right:5px;color:darkgreen;"><img src="wt_project/user/views/media/website_images/add_user.svg" width="70px;" height="50px;" style="margin-left: 15vw;"> Add Company</a>
	...
	font-weight: bold; width: 60vw; padding:2px; background-color: #34495E  ; font-size: 1.2rem; ">
				  ...
				  <div class="card-body">
						<?php
						for ($i=0; $i <count($company_management_detail) ; $i++) {
						//	echo $company_management_detail[$i]["company_id"];
							//echo $company_management_detail[$i]["name"];
							//echo $company_management_detail[$i]["designation"];


            echo '<form class="" action="?act=update_company_management_detail" method="post">

				  	<div class="input-group mb-3">
  					  <div class="input-group-prepend">
  					    <span class="input-group-text" id="basic-addon1">Comapany ID</span>
  					  </div>
					    <input type="text" class="form-control" name="company_id"  aria-describedby="basic-addon1" value='.$company_management_detail[$i]["company_id"].'>
					  </div>
						<div class="input-group mb-3">
  					  <div class="input-group-prepend">
  					    <span class="input-group-text" id="basic-addon1">Member Name</span>
  					  </div>
					    <input type="text" class="form-control" name="name"  aria-describedby="basic-addon1" value='.$company_management_detail[$i]["name"].'>
					  </div>
						<div class="input-group mb-3">
  					  <div class="input-group-prepend">
  					    <span class="input-group-text" id="basic-addon1">Member Designation</span>
  					  </div>
					    <input type="text" class="form-control" name="designation"  aria-describedby="basic-addon1" value='.$company_management_detail[$i]["designation"].'>
					  </div>
        <button class="btn btn-success" style="float: left;margin-bottom:20px;" type="submit" class="btn btn-primary" name="management_id" value='.$company_management_detail[$i]["Id"].'>
          Update and Save Details
        </button>
        </form>
          <form class="" action="?act=delete_comapny_management_detail" method="post">
          <button type="submit" class="btn btn-danger" name="management_id"  style="float: right;margin-bottom:20px;" value='.$company_management_detail[$i]["Id"].'>
            Delete Details
          </button>
          </form>';
				}
